...
//static void ck_hash_table_put(struct ck_hash_table *ct, char *key, char *val, int n )

/*int main(int argc, char *argv[])
{
	char keys[100][MAX_KEY_SIZE];

	char vals[100][MAX_VAL_SIZE];
	struct ck_hash_table *ct = ck_hash_table_init();

	int i = 0;
	for(i = 0; i < 100; i++) {
		snprintf(keys[i], MAX_KEY_SIZE-1, "key%d", (i+3)*3);
	}

	for(i = 0; i < 100; i++) {
		snprintf(vals[i], MAX_VAL_SIZE-1, "val%d", (i+3)*3);
		ck_hash_table_insert(ct, keys[i], vals[i]);
	}

	
	for(i = 0; i < 100; i++) {
		char *res = ck_hash_table_get(ct, keys[i]);
		if(!res) {
			printf("could not find entry %s\n", keys[i]);
		} else{
			printf("GET result: %s: %s\n", keys[i], res);
		}
	}
	

	//ck_hash_print_entries(ct);
